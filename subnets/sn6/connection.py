...
class SN6Connection:
    ...
    def upload_agent(self, agent_name: str, track: str, agent_file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            data = {"name": agent_name, "track": track}

            files = {"agent_file": (agent_file_path.split("/")[-1], file_content, "text/x-python")}
            ...
            if response.status_code == 200:
                ...
                bt.logging.error(
                    f"Failed to upload agent: {response.status_code} - {response.text}"
                )
