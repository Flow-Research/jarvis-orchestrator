...
This is the official external skill for Garden personal operator agents working through the Flow Workstream API.

The agent uses Workstream to:
...
- inspect quality counters for the authenticated Garden user
...
- Put source-native post id, author/username, text, URL, and timestamp in `content` when the contract requires them.
- Match hashtags or keywords against source-native text, not against your own notes.
- Use the canonical post URL as `uri`.

For Reddit-like tasks:

- Put subreddit, post/comment id, author/username, title/body or text, URL, and created timestamp in `content` when the contract requires them.
- Match subreddit targets exactly. A task for `r/example` must not receive records from a different subreddit.
- Use the canonical Reddit item URL as `uri`.

For unknown or future task sources:

- Do not infer schema from prior sources.
- Read `contract.source_requirements.required_content_fields`.
- If the required fields are unclear or unsupported by your tools, skip the task.
